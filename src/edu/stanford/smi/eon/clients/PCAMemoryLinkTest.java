...
public class PCAMemoryLinkTest extends Object {
	...
	static Logger logger = Logger.getLogger(PCAMemoryLinkTest.class);

	public static void main(String[] argv) {
		PCAMemoryLinkTest pCATest = new PCAMemoryLinkTest();
		printProperties();
...
//		*/
		...
		while (count < 1000 ) {
			...
			data[1].numeric(new Numeric_Data(Data_Operation_Type.add,
					"MD_Typical_Diastolic_BP",  "mm/mm", "2005-02-14", "95"));
			data[2] = new Patient_Data();
			data[2].prescription(new Prescription_Data(Data_Operation_Type.add,
					"lisinopril",  50, "", "", "", ""));
			data[3] = new Patient_Data();
			data[3].note_data(new Note_Entry_Data(Data_Operation_Type.add,
					Entry_Type.problem, "HIV_Test",  "POS", "2007-12-14", "")); 
...
//				dssOutput =pca.computeAndStoreAdvisories(outputdir);
			...
			Collection instances = PCAImp.kbManager.getKB().getInstances();
			logger.info("Number of Protege instances = "+instances.size());
			try {
			...
			logger.info("TestPadda.Main count : "+count);
/*			try {
				line = reader.readLine();
