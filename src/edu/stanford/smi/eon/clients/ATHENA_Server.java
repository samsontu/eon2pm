 ...
 * April, 2012
 * 
 * Code updated for ATHENA HF RRP project by Samson Tu
 * November, 2013
 */
...
public class ATHENA_Server extends Object {
	...
	static String htmlDir = "/usr/sandbox/html/";
	
	public ATHENA_Server() {
		//Class constructor code can go here. 
	};

	private PCAServer_i PCAImp = null;
	private static ATHENA_Server Athena_Server_Obj = null;
	static PCASession pca = null;

	static Logger logger = Logger.getLogger(ATHENA_Server.class);
	...
	private String[] loadKBInfo(String mode, String parentPath) {
		...
		for (int i=0; i < sl.Count(); i++) {
			...
			if (lblParts[1].equals("KB")) {
				String kbFilePath = parentPath + "kb/" + s.split("=")[1].trim();   
				results[0] = kbFilePath;
	...
	private static void processFile(String pid, File child, File outFilePath, String guidelineId, String HTMLFileName) {
		...
		try {
			System.out.println("-----------------------------------------");
			recommendations = pca.topLevelComputeAdvisory( pid, caseData,  HelperFunctions.internalDateFormatter.format(startTime), guidelineId, HTMLFileName );
			System.out.println("-----------------------------------------");
	...
	public static void main(String[] argv) throws InterruptedException {
		...
		Athena_Server_Obj = new ATHENA_Server();
		...
		File inFilePath  = new File(parentPath + "data/");
		File outFilePath = new File(parentPath + "output/");
		File kbFilePath  = new File(parentPath + "kb/"); 
		...
		while (mainLoopRunning) {
			...
			for (int i=0; i < listOfFiles.length; i++) {
				...
				if (finishFileExists == true) {
					...
					String htmlFileName = outFilePath.getPath()+"/"+mode+"_"+patientID+".html";
					File htmlFileDir = new File(htmlDir);
					if (!htmlFileDir.exists()) 
						htmlFileName = htmlDir +mode+"_"+patientID+".html";
					System.out.println("SENDING TO: ["+outFileName.getPath()+"]");
					System.out.println("CREATING HTML FILE: "+htmlFileName);
					String readyFileName;
				    readyFileName = outFileName.getPath();
					try { 
						processFile(patientID, child, outFileName, kbID, htmlFileName);
					} catch (Throwable t) {
						try {
